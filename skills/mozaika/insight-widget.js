(() => {
  ...
  const DEFAULT_TASK_PREFIX = 'Найди в данных важные для руководителя инсайты: изменения, риски и возможности.';
  const FIXED_TASK_SUFFIX = 'Подготовь проверенные выводы, наглядные визуализации, варианты истории и итоговую HTML-презентацию.';
  const CONFIG = {
    route: '/api/extensions/mozaika/scenario/insight/start',
    pickerRoute: '/api/extensions/mozaika/scenario/local/pick',
    presetsRoute: '/api/extensions/mozaika/scenario/task-presets',
    choicePendingRoute: '/api/extensions/mozaika/scenario/choice/pending',
    choiceAnswerRoute: '/api/extensions/mozaika/scenario/choice/answer',
    title: 'Данные для поиска инсайтов',
    lead: 'Добавьте URL, отдельные файлы или целую папку — источники можно смешивать.',
    taskLabel: 'Описание задачи',
    defaultTask: `${DEFAULT_TASK_PREFIX} ${FIXED_TASK_SUFFIX}`,
    startLabel: 'Найти инсайты',
    taskField: 'executive_brief',
  };
  ...
  root.innerHTML = `
    <style>
      * { box-sizing: border-box; }
      ...
      .choice-panel { min-height: 286px; padding: 12px 14px; background: #faf9f5; }
      .choice-panel[hidden], #launch-panel[hidden] { display: none; }
      .choice-head { display: flex; align-items: flex-start; justify-content: space-between; gap: 12px; margin-bottom: 9px; }
      .choice-kicker { display: inline-flex; align-items: center; min-height: 22px; margin-bottom: 4px; padding: 3px 9px; border-radius: 999px; background: #e8f0ed; color: #236a57; font-size: 11px; font-weight: 750; letter-spacing: .02em; }
      .choice-title { margin: 0; font-size: 18px; line-height: 22px; }
      .choice-question { margin: 3px 0 0; color: #5e5d59; font-size: 13px; }
      .choice-count { flex: 0 0 auto; padding-top: 4px; color: #77756f; font-size: 11px; white-space: nowrap; }
      .choice-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 8px; }
      .choice-card { position: relative; display: grid; grid-template-rows: auto auto auto 1fr auto auto; gap: 5px; min-width: 0; min-height: 202px; padding: 11px; border: 1px solid #d7d3ca; border-radius: 13px; background: #fff; box-shadow: 0 7px 20px #1414130d; transition: transform .15s ease, border-color .15s ease, box-shadow .15s ease; }
      .choice-card:hover, .choice-card:focus-within { transform: translateY(-2px); border-color: #388f76; box-shadow: 0 10px 24px #14141318; }
      .choice-card.recommended { border-color: #388f76; background: linear-gradient(180deg, #f4faf7 0, #fff 42%); }
      .choice-badge { min-height: 18px; color: #236a57; font-size: 10px; font-weight: 800; text-transform: uppercase; letter-spacing: .05em; }
      .choice-card h3 { margin: 0; font-size: 15px; line-height: 18px; }
      .choice-thought { margin: 0; color: #5e5d59; font-size: 11px; line-height: 15px; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden; }
      .choice-beats { display: grid; gap: 3px; align-content: start; margin: 0; padding: 0; list-style: none; }
      .choice-beats li { position: relative; padding-left: 10px; font-size: 10px; line-height: 13px; color: #343330; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
      .choice-beats li::before { content: ""; position: absolute; left: 0; top: 5px; width: 4px; height: 4px; border-radius: 50%; background: #388f76; }
      .choice-impact { margin: 0; padding-top: 5px; border-top: 1px solid #ece9e1; color: #236a57; font-size: 10px; line-height: 13px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
      .choice-action { width: 100%; min-height: 31px; border: 0; border-radius: 8px; padding: 6px 9px; background: #141413; color: #fff; font-size: 11px; font-weight: 750; cursor: pointer; }
      .choice-action:hover, .choice-action:focus-visible { background: #388f76; outline: none; }
      .choice-action:disabled { opacity: .55; cursor: wait; }
      .choice-status { min-height: 18px; margin-top: 5px; color: #5e5d59; font-size: 11px; text-align: right; }
      .choice-status.error { color: #9f2d2d; }
      .choice-status.ok { color: #388f76; }
    ...
    <section id="choice-panel" class="choice-panel" hidden aria-live="polite">
      <div class="choice-head">
        <div>
          <div class="choice-kicker">Нужен ваш выбор</div>
          <h2 id="choice-title" class="choice-title"></h2>
          <p id="choice-question" class="choice-question"></p>
        </div>
        <div id="choice-count" class="choice-count"></div>
      </div>
      <div id="choice-grid" class="choice-grid"></div>
      <div id="choice-status" class="choice-status"></div>
    </section>
    <main id="launch-panel">
      <div class="top">
  ...
  function setChoiceStatus(message, kind = '') {
    choiceStatus.textContent = message;
    choiceStatus.className = `choice-status ${kind}`;
  }

  function choiceRenderKey(question, count) {
  ...
  function makeChoiceCard(question, option) {
    const recommended = option.id === question.recommended_option_id;
    const card = document.createElement('article');
    card.className = `choice-card${recommended ? ' recommended' : ''}`;

    const badge = document.createElement('div');
    badge.className = 'choice-badge';
    badge.textContent = recommended ? 'Рекомендация Mozaika' : 'Вариант истории';

    const headline = document.createElement('h3');
    headline.textContent = option.headline;

    const thought = document.createElement('p');
    thought.className = 'choice-thought';
    thought.textContent = option.main_thought;

    const beats = document.createElement('ul');
    beats.className = 'choice-beats';
    (option.story_beats || []).forEach((text) => {
    ...
    impact.textContent = option.executive_implication;
    impact.title = option.executive_implication;

    const choose = document.createElement('button');
    choose.type = 'button';
    choose.className = 'choice-action';
    choose.textContent = recommended ? 'Выбрать рекомендацию' : 'Выбрать эту историю';
    choose.addEventListener('click', () => submitOwnerChoice(question, option));
    card.append(badge, headline, thought, beats, impact, choose);
    return card;
  }
  ...
  function renderOwnerChoice(question, pendingCount) {
    const key = choiceRenderKey(question, pendingCount);
    if (key === activeChoiceKey) return;
    activeChoiceKey = key;
    choiceTitle.textContent = question.title;
    choiceQuestion.textContent = question.question;
    choiceCount.textContent = pendingCount > 1 ? `Ещё решений: ${pendingCount - 1}` : '';
    choiceGrid.replaceChildren(...question.options.map((option) => makeChoiceCard(question, option)));
    setChoiceStatus('Выбор сразу вернётся в текущую задачу.');
    launchPanel.hidden = true;
    choicePanel.hidden = false;
  }

  async function loadPendingChoices() {
    if (choiceSubmitting) return;
    try {
      const response = await fetch(CONFIG.choicePendingRoute, {
        headers: { Accept: 'application/json' },
        cache: 'no-store',
      });
      const data = await response.json();
      if (!response.ok || !data.ok || !Array.isArray(data.questions)) throw new Error(data.error || `HTTP ${response.status}`);
      if (!data.questions.length) {
        showLaunchPanel();
        return;
      }
      renderOwnerChoice(data.questions[0], data.questions.length);
    } catch (error) {
      if (!choicePanel.hidden) setChoiceStatus(`Не удалось обновить варианты: ${error.message || error}`, 'error');
    }
  }

  async function submitOwnerChoice(question, option) {
    if (choiceSubmitting) return;
    choiceSubmitting = true;
    choiceGrid.querySelectorAll('button').forEach((button) => { button.disabled = true; });
    setChoiceStatus('Сохраняю выбор и возвращаю его Mozaika…');
    try {
      const response = await fetch(CONFIG.choiceAnswerRoute, {
        method: 'POST',
      ...
      setChoiceStatus(`Выбрано: ${option.headline}. Задача продолжает работу.`, 'ok');
      setTimeout(() => {
        choiceSubmitting = false;
        activeChoiceKey = '';
        void loadPendingChoices();
      }, 1100);
    } catch (error) {
      choiceSubmitting = false;
      choiceGrid.querySelectorAll('button').forEach((button) => { button.disabled = false; });
      setChoiceStatus(error.message || String(error), 'error');
    }
  }
  ...
  void loadPendingChoices();
  window.setInterval(() => { void loadPendingChoices(); }, 1800);
})();
